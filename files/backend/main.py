    ...
    sample = [
        {
            ...
            "poster_url": "https://via.placeholder.com/320x480.png?text=Inception",
        },
        {
        ...
        {
            "title": "A Generic B-Movie",
            "overview": "A low-budget story with thin dialogue and recycled plot points.",
            "genres_list": ["Romance"],
            "year": 2012,
            "language_str": "English",
        },
        {
            "title": "Pure Cheesefest",
            "overview": "A forgettable hour and a half of cliché romance and bad acting.",
        ...
        },
        {
            ...
            "year": 1994,
            "language_str": "English",
            "poster_url": "https://via.placeholder.com/320x480.png?text=Shawshank",
        },
        {
            "title": "Dune",
...
def find_movie(title: str):
    ...
    if mask2.any():
        idx = df[mask2].index[0]
        print(f"✅ Partial match found at index {idx}")
        return df.loc[idx], idx
...
def row_to_dict(row):
    ...
    if poster is None or (isinstance(poster, float) and pd.isna(poster)):
        poster = "https://via.placeholder.com/320x480.png?text=No+Image"
...
def search(title: str):
    if df is None:
        return {"error": "Dataset not loaded", "available_movies": []}
    if not title or len(title.strip()) < 1:
        return {"error": "Title parameter is required", "available_movies": df["title"].tolist()}

    row, _ = find_movie(title)
    if row is None:
        return {"error": "Movie not found", "available_movies": df["title"].tolist()}
...
def recommend(title: str):
    ...
    try:
        ...
        return {"error": "Internal error during recommendation", "available_movies": df["title"].tolist()}
...
def verdict(title: str):
    ...
    try:
        ...
        # A) Overview Quality (0–2 points)
        overview_len = len(row["overview"])
        if overview_len > 200:
            score += 2.0
            reasons.append("excellent overview depth")
        elif overview_len > 100:
            score += 1.0
            reasons.append("good overview length")
        else:
            reasons.append("short overview")

        # B) Genre Strength (0–1 point)
        genres_lower = {g.lower() for g in (row.get("genres_list") or [])}
        popular_genres = {"action", "drama", "thriller", "sci-fi", "science fiction"}
        if genres_lower & popular_genres:
            score += 1.0
            reasons.append("popular genre match")
        else:
            reasons.append("niche genre")

        # C) Similarity Confidence (0–1 point)
        movie_vec = tfidf_matrix[idx]
        sim_scores = cosine_similarity(movie_vec, tfidf_matrix).flatten()
        sim_scores[idx] = -1  # exclude self
        top_indices = np.argsort(sim_scores)[::-1][:5]
        avg_sim = np.mean(sim_scores[top_indices]) if len(top_indices) > 0 else 0.0
        if avg_sim > 0.20:
            score += 1.0
            reasons.append("high similarity confidence")
        elif avg_sim > 0.15:
            score += 0.5
            reasons.append("moderate similarity confidence")
        else:
            reasons.append("low similarity confidence")

        # D) Diversity Boost (0–1 point)
        top_titles = [df.iloc[i]["title"] for i in top_indices if i < len(df)]
        if len(set(top_titles)) == len(top_titles):  # no duplicates
            score += 1.0
            reasons.append("diverse recommendations")
        else:
            reasons.append("some duplicate recommendations")

        # E) Classic title boost (top known classics get additional strength)
        if row["title"].strip().lower() in {"the godfather", "the shawshank redemption"}:
            score += 2.0
            reasons.append("legendary classic boost")
        elif row["title"].strip().lower() in TOP_CLASSICS:
            score += 1.0
            reasons.append("top classic boost")

        score = min(5.0, round(score, 2))

        if score >= 4:
            verdict_text = "🔥 Worth Watching"
        elif score >= 2:
            verdict_text = "👍 Decent"
        else:
            verdict_text = "❌ Not Worth Watching"
        ...
        print(f"❌ Error in verdict: {e}")
        return {"error": "Internal error during verdict calculation", "available_movies": df["title"].tolist()}
